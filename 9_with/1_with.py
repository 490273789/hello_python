...
工作原理:
1.使用with关键字系统会自动调用f.close() 方法，with的作用等校于try finally语句
2.当执行with语句时，会调用expression对象的__enter__方法。__enter__方法的返回值可以被存储在variable中，以供with代码块中使用。
3.然后执行with语句内部的代码块
4.无论代码块中是否发生异常，都会调用expression对象中的__exit__方法，以确保资源的释放或清理工作，这类似于try finally中的finally子句
"""
